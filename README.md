...
- **The repo is private.** The footer's GitHub link points at `github.com/tizzyeth/wizard-tower`, which **404s for visitors until you make the repo public** (Settings → General → Change visibility). Either make it public — appropriate for a "community-built" dashboard whose credibility rests on being auditable — or point `LINKS.github` in `config/token.ts` somewhere else.
