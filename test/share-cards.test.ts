...
describe("share card slugs", () => {
  it("accepts only the four modules that stand alone as an image", () => {
    expect([...SHARE_CARDS]).toEqual(["verdict", "ledger", "holders", "wards", "flow"]);
    for (const slug of SHARE_CARDS) expect(isShareCard(slug)).toBe(true);
  });

  it("rejects anything else, so /share/<junk> cannot render", () => {
    // The chart and the trade tape stay absent: flattened to a still image they
    // say less than the screenshot someone would take anyway.
    for (const slug of ["chart", "tape", "", "../secret", "VERDICT"]) {
